...
export class ActiveOrdersService {
  ...
  fetchActiveOrdersOf(userId: number) {
    ...
    this.http.get<ActiveOrderModel[]>(
      BaseUrl.B1 + '/api/v1/customer/active_orders?id=' + userId, httpOptions
    ).subscribe((data: ActiveOrderModel[]) => {
        this.activeOrders = data.sort((n1, n2) => n1.symbol.localeCompare(n2.symbol));
      }, error => console.log(error)
    );
  ...
  cancelOne(clOrdID: string) {
    ...
    this.http.delete<any>(
      BaseUrl.B1 + '/api/v1/trade/order?clOrdID=' + clOrdID, httpOptions
    ).subscribe(
      () => {
  ...
  cancelAll(symbol: string) {
    ...
    this.http.delete<any>(
      BaseUrl.B1 + '/api/v1/trade/order?symbol=' + symbol, httpOptions
    ).subscribe(
      () => {
